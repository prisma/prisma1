...
export default class ModelWhereInputGenerator extends RelationalModelWhereInputGenerator {
  ...
  public generateManyRelationFilterFields(field: IGQLField): GraphQLInputFieldConfigMap {
    ...
    const restrictedWhereType = this.generators.modelRestrictedWhereInput.generate(fieldType, {})

    if(fieldType.isEmbedded) {
      return {
        ...RelationalModelWhereInputGenerator.generateFiltersForSuffix(['_some'], field, whereType),
        // In this special case, we switch to the restricted where type.
        ...RelationalModelWhereInputGenerator.generateFiltersForSuffix(['_every', '_none'], field, restrictedWhereType)
      }
    } else {
      return RelationalModelWhereInputGenerator.generateFiltersForSuffix(['_some'], field, whereType)
    }
  }
