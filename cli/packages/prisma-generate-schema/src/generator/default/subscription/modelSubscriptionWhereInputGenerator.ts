...
export default class ModelSubscriptionWhereInput extends ModelInputObjectTypeGenerator {
  ...
  protected generateFields(model: IGQLType, args: {}): GraphQLInputFieldConfigMap {
    let fields = {
      ...
      'updatedFields_contains_every': { type: new GraphQLList(new GraphQLNonNull(GraphQLString)) },
      'updatedFields_contains_some': { type: new GraphQLList(new GraphQLNonNull(GraphQLString)) },
