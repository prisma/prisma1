  ...
  return new Promise((resolve, reject) => {
    fs.mkdir(p, (err) => {
      ...
      lockActive(pidPath).then(active => {
        if (!active) return unlock(p).then(resolve as any).catch(reject)
...
async function getActiveReaders (path: string, timeout: number, skipOwnPid: boolean = false): Promise<number[]> {
  ...
  return skipOwnPid ? activeReaders.filter(r => r !== process.pid) : activeReaders as any
}
...
export async function hasReaders (p: string, options: WriteLockOptions = {}): Promise<boolean> {
  let timeout = options.timeout || 60000
  let skipOwnPid = !!options.skipOwnPid
