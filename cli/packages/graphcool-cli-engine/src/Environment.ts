...
export class Environment {
  ...
  checkCluster(cluster: string) {
    const allClusters = this.config.sharedClusters.concat(
      Object.keys(this.rc.clusters || {}),
    )
...
`)
      ...
      const bestMatch = stringSimilarity.findBestMatch(cluster, allClusters)
        .bestMatch.target
      this.out.error(
        `${cluster} is not a valid cluster. Did you mean ${bestMatch}?`,
      )
    }
  }
  ...
  getTargetWithName(
    targetName?: string,
  ): { target: Target | null; targetName: string | null } {
    let target: any = null
  ...
  migrateClusters(rcPath: string) {
    if (fs.pathExistsSync(rcPath)) {
      ...
      try {
        ...
        if (
          content.clusters &&
          Object.keys(content.clusters).find(c => content.clusters[c].token)
        ) {
          const newRcJson = {
            ...content,
            clusters: mapValues(
          ...
          this.out.warn(`We detected the old definition format of ${chalk.bold(
            'clusters',
          )} in the ${rcPath} file.
...
`)
        ...
        if (
          content.clusters &&
          Object.keys(content.clusters).find(
            c =>
              typeof content.clusters[c] !== 'string' &&
              !content.clusters[c].faasHost,
          )
        ) {
          const newRcJson = {
            ...content,
          ...
          fs.writeFileSync(rcPath, newLocalRcYaml)
          this.out.warn(
            `We detected the old definition format of ${chalk.bold(
              'clusters',
            )} in the ${rcPath} file. A new field called ${chalk.bold(
              'faasHost',
...
`)
  ...
  async load(args: Args) {
    const localFile =
      this.config.localRCPath && fs.pathExistsSync(this.config.localRCPath)
        ? fs.readFileSync(this.config.localRCPath, 'utf-8')
        : null
    const globalFile =
  ...
  async loadRCs(
    ...
    if (this.rc.clusters && this.rc.clusters.default) {
      if (!this.allClusters.includes(this.rc.clusters.default)) {
        this.out.error(
          `Could not find default cluster ${this.rc.clusters.default}`,
        )
      }
  ...
  deserializeRCs(
    ...
    newGlobalFile.targets = this.resolveTargetAliases(
      newGlobalFile.targets,
      newGlobalFile.targets,
    )
    ...
    for (let i = 0; i < 2; i++) {
      ...
      newLocalFile.targets = this.resolveTargetAliases(
        newLocalFile.targets,
        allTargets,
      )
    ...
    newLocalFile.targets = this.deserializeTargets(
      newLocalFile.targets,
      localFilePath,
    )
    newGlobalFile.targets = this.deserializeTargets(
      newGlobalFile.targets,
      globalFilePath,
    )
    // check if clusters exist
    const allClusters = [
      ...this.config.sharedClusters,
      ...Object.keys(newGlobalFile.clusters || {}),
      ...Object.keys(newLocalFile.clusters || {}),
    ]
  ...
  checkClusters(targets: Targets, clusters: string[], filePath: string | null) {
    Object.keys(targets).forEach(key => {
      ...
      if (
        !clusters.includes(target.cluster) &&
        !this.warningCache[target.cluster]
      ) {
...
Please run ${chalk.bold('graphcool local up')} to start the local cluster.`)
  ...
  deserializeTargets(
    ...
    return mapValues<string, Target>(targets, target =>
      this.deserializeTarget(target, filePath),
    )
  }
  ...
  resolveTargetAliases = (targets, allTargets) =>
    mapValues(
      targets,
      target =>
        this.isTargetAlias(target)
          ? this.resolveTarget(target, allTargets)
          : target,
    )
