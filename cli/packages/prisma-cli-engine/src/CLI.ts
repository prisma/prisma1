   ...
   if (foundCommand) {
    ...
    if (topic) {
     ...
     throw new CommandReplacedError('seed', 'import')
      ...
      throw new CommandRemovedError('1.7', 'https://bit.ly/release-notes-1-7', '')
    } else if (
      [
        'local',
        'local:up',
      ...
      throw new CommandRemovedError('1.7', 'https://bit.ly/release-notes-1-7', 'You can use docker-compose directly to manage the state of a local server.')
    } else {
  ...
  if (
   !(
    ...
    (this.config.argv.includes('deploy') &&
     (this.config.argv.includes('-w') || this.config.argv.includes('--watch')))
   )
  ) {
