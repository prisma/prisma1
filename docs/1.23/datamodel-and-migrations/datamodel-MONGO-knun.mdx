...
> This example is absed the on [datamodel v1.1](b6a7) format which is currently in [Preview](oje2#preview-vs-final).  
...
Now you can run `prisma1 generate` and the Prisma CLI will stored the Prisma GraphQLS schema in `./prisma-schema/prisma.graphql`.
