...
Select your database on the code block and put the following contents into the Docker Compose file you just created:
      ...
      PRISMA_CONFIG: |
        ...
        databases:
          default:
            ...
            database: __YOUR_MONGODB_DATABASE__
```
...
<Collapse title="See introspection command for PostgreSQL">

```bash copy
prisma introspect \\
  --pg-host __YOUR_POSTGRES_HOST__ \\
  --pg-port __YOUR_POSTGRES_PORT__ \\
  --pg-db __YOUR_POSTGRES_DB__ \\
  --pg-schema __YOUR_POSTGRES_SCHEMA__ \\
  --pg-user __YOUR_POSTGRES_USER__ \\
  --pg-password __YOUR_POSTGRES_PASSWORD__
```

If your PostgreSQL server uses SSL, you need to append the `--pg-ssl` flag as well. The placeholders you need to replace in the command are identical to the ones from above:
...
<Collapse title="See introspection command for MongoDB">

```bash copy
prisma introspect \\
  --mongo-uri __YOUR_CONNECTION_STRING__ \\
  --mongo-db __YOUR_MONGODB_DATABASE__
```

The placeholders you need to replace in the command are identical to the ones from above:

- `__YOUR_CONNECTION_STRING__`: Your MongoDB connection string, e.g. `http://user1:myPassword@localhost:27017/admin`. Learn more [here](b6o5#prisma_config). Note that this must include the database credentials as well as the [`authSource`](https://docs.mongodb.com/manual/reference/connection-string/#authentication-options) database that's storing the credentials of your MongoDB admin user (by default it is often called `admin`).
- `__YOUR_MONGODB_DATABASE__`: The name of your MongoDB [database](https://docs.mongodb.com/manual/core/databases-and-collections/#databases).

</Collapse>

The CLI generates the `datamodel-[TIMESTAMP].prisma` file containing the SDL version of your database schema. On the first run, it also writes the `datamodel` property into the `prisma.yml`.
