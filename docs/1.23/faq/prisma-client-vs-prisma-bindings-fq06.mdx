...
Prisma bindings are installed into your application using the `prisma-binding` package from NPM. Its API _mirrors_ the [Prisma GraphQL API](/prisma-graphql-api).
...
The Prisma client is auto-generated using the [`prisma1 generate`](xcv2) command of the Prisma CLI. The Prisma CLI reads the information from your `prisma.yml` and generates the according Prisma client.
...
Running `prisma1 generate` with this configuration generates three versions of the Prisma client, one in JavaScript, one in TypeScript and one in Go. Note that you typically only generate the Prisma client in one language.
