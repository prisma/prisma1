...
Therefore, when starting out with an existing MongoDB, it is extremely important that the legacy data is consistent and that it can be modeled accurately as a Prisma datamodel. You can use the [`prisma introspect`](https://github.com/prisma/prisma/issues/3529) command to derive a datamodel from the data that's currently stored in your MongoDB.å
...
You can use [`prisma introspect`](https://github.com/prisma/prisma/issues/3529) to suggest a datamodel.å
