...
`prisma introspect` works in a similar way as the `prisma1 init` wizard in that you need to provide the database connection information.

While `prisma1 init` wizard generates an entire _service configuration_, `prisma introspect` only generates the datamodel file:
