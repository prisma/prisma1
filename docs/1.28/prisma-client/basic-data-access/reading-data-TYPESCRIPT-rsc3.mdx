...
For this page, we'll assume your Prisma project is based on the following [datamodel](knul):
...
Whenever a database record is queried using the Prisma client, _all scalar fields_ of that record are fetched. This is true no matter if a single record or a list of records is queried.

For example, the following query returns all scalar fields of a single `User`:

<Code languages={["TypeScript", "Response"]}>

```ts
const user: User = await prisma.user({ email: "bob@prisma.io" })
```
...
In this case, the returned `user` record will have four properties (that correspond to the scalar fields of the `User` model): `id`, `name`, `email` and `role`. The `posts` and `comments` fields are both _relation fields_ and are therefore not included in the response.
...
The method is named after the model but starts with a _lowercase_ character. For the sample datamodel from above, the three methods for fetching single `User`, `Post` and `Comment` records are called `user`, `post` and `comment`.
...
```ts
const post: Post = await prisma.post({ id: "cjlgpyueg001o0a239d3i07ao" })
```
...
```ts
const user: User = await prisma.user({ email: "ada@prisma.io" })
```
...
For each model in your datamodel, there is a method generated in the Prisma client API that allows to fetch a list of the respective records. The method is named after the model but starts with a _lowercase_ character and uses the _plural_ form. For the sample datamodel from above, the three methods for fetching lists of `User`, `Post` and `Comment` models are called `users`, `posts` and `comments`.
...
```ts
const comments: Comment[] = await prisma.comments()
```
...
In this example, `users()` already returns a list, so it's not possible to query the `posts` relation of each user object inside the list.

</Warning>

### Examples

_Query all the posts of a single user_:
...
```
 ...
 _Query all the comments for a certain post_:

<Code languages={["TypeScript", "Response"]}>

```ts
const commentsForPost: Comment[] = await prisma
...
```ts
const usersWithAInName: User[] = await prisma
  .users({
    where: { name_contains: "A" }
  })
```
...
```ts
const usersCalledAdaOrGrace: User[] = await prisma
  .users({
    where: {
      name_in: ["Ada", "Grace"]
    }
...
```ts
const commentsBeforeChristmas: Comment[] = await prisma
  .comments({
...
```ts
const filteredPosts: Post[] = await prisma
  .posts({
    where: {
      AND: [
        {
...
```ts
const postsByUserBeforeChristmas: LinkNode[] = await prisma
  .user({ email: "ada@prisma.io" })
  .posts({
    where: {
...
```ts
const sortedComments: Post[] = await prisma
...
```ts
const sortedUsers: User[] = await prisma
...
_Query the first 3 records (seeking forward)_:
...
```ts
const posts: Post[] = await prisma
  .posts({ first: 3 })
```

</Code>

_Query the records from position 6 to position 10 (seeking forward)_:
...
_Query the last 3 records (seeking backward)_:
...
_Query the records from position 21 to position 27 (seeking backward)_:
...
_Query the first 3 records after the record with `cixnen24p33lo0143bexvr52n` as `id`_:
...
_Query the first 5 recrods after the object with `cixnen24p33lo0143bexvr52n` as `id` and skipping 3 records_:
...
_Query the last 5 records before the record with `cixnen24p33lo0143bexvr52n` as `id`_:
...
_Query the last 3 records before the record with `cixnen24p33lo0143bexvr52n` as `id` and skipping 5 records_:
...
const result: any = await prisma.$graphql(query)
```
...
`
const variables: UserWhereUniqueInput = { id: 'cjsviilie0g880b43twzd93li' }
