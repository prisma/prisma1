...
Running [`prisma1 generate`](xcv2) in the directory where that `prisma.yml` is located generates a Prisma client in JavaScript and stores the generated files in a directory called `prisma-client`.
