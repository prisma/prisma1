...
After setting `managementApiSecret` run `docker-compose up -d` to recreate the containers. Note that this will create a new, empty database for your server. After that, you'll need to use the environment variable `PRISMA_MANAGEMENT_SECRET` to authenticate with the management api from the command line. For example, to deploy your server after changes run `PRISMA_MANAGEMENT_SECRET=__YOUR_MANAGEMENT_API_SECRET prisma deploy`.
