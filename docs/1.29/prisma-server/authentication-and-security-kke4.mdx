...
When running `prisma generate` or `prisma1 deploy`, the Prisma CLI will read the `MY_SECRET` environment variable. Learn more about using environment variables in `prisma.yml` [here](5cy7#environment-variable).
