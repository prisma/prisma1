...
export const meta = {
  title: 'Generating the Client (Go)',
  position: 20,
  technology: 'go',
  technologyOrder: 3,
...
Running [`prisma1 generate`](xcv2) in the directory where that `prisma.yml` is located generates a Prisma client in Go and stores it in a directory called `prisma-client`.
