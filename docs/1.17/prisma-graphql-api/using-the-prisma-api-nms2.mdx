...
The [`graphql-request`](https://github.com/prisma/graphql-request) library is a lightweight wrapper on top of `fetch` which you can use to save writing boilerplate code. It is mostly used in scripts and smaller applications that need to talk to a GraphQL API.

### Using `request`

<Warning>

`request` does not support passing _headers_ to the request ([yet](https://github.com/prisma/graphql-request/pull/33)). Therefore, this particular example assumes your Prisma service was deployed without a service secret.
