...
The Prisma CLI has a hidden helper command for generating Management API tokens: `prisma1 cluster-token`. Before running it, you need to set the `MANAGEMENT_API_SECRET` environment variable. Note that this command is not officially documented and might change without further notice.
