...
1. Select **Prisma Go Client** to generate Prisma client for Go.
...
Run the following command to create an empty Go script:
...
Add the following code in `main.go` and save it afterwards:
...
func main() {
  ...
  fmt.Println("Created new user:", newUser.name, "ID:", newUser.id)

  allUsers, err := client.users().Exec(ctx)
...
```go lineNumbers,copy
user, err := client.user(prisma.UserWhereInput{
  Id: "__USER_ID__",
}).Exec(ctx)
```

```go lineNumbers,copy
users, err := client.users(prisma.UserWhereInput{
  Name: "Alice",
}).Exec(ctx)
```

```go lineNumbers,copy
users, err := client.updatedUser(prisma.UserUpdateInput{
  Name: "Alice",
}).Exec(ctx)
...
```go lineNumbers,copy
deletedUser, err := client.deleteUser(prisma.UserDeleteInput{
  Id: "__USER_ID__",
}).Exec(ctx)
