...
- [**Types**](#object-types) consist of multiple [fields](#fields) and typically represent entities from your application domain (e.g. `User`, `Car`, `Order`). Each type in your datamodel is mapped to a database table and CRUD operations are exposed in the generated Prisam client API.
...
- `createdAt: DateTime! @createdAt`: Stores the exact date and time for when a node of this object type was _created_.
- `updatedAt: DateTime! @updatedAt`: Stores the exact date and time for when a node of this object type was _last updated_.
