...
Paste the HTTP endpoint for the `Simple API` that you saved after running `graphcool-framework deploy` into `./src/main.js` as the `uri` argument in the `createNetworkInterface` call:
