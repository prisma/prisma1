...
Now that the CLI is installed, you can use it to create a new service with the [`graphcool-framework init`](!alias-zboghez5go#graphcool-init) command.
...
Paste the HTTP endpoint for the `Simple API` that you saved after running `graphcool-framework deploy` into `./App.js` as the `uri` argument in the `HttpLink` constructor call:
