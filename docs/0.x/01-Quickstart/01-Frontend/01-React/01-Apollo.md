...
Paste the HTTP endpoint for the `Simple API` that you saved after running `graphcool-framework deploy` into `./src/index.js` as the `uri` argument in the `HttpLink` constructor call:
