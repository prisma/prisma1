...
Paste the HTTP endpoint for the `Simple API` that you saved after running `graphcool-framework deploy` into `AppDelegate.swift` as the `uri` argument in the `createNetworkInterface` call:
