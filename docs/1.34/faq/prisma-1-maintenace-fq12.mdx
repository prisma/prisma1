...
2. Inspect `prisma.yml` and any environment variable your build process is using.
3. If your project uses a `PRISMA_MANAGEMENT_API_SECRET`, you are **not** using Prisma 1 Cloud. You can verify this by using `prisma1 logout` from the command line and attempting to deploy or access a service.
...
4. Confirm deletion. **Note: Deleting a service following the steps below will delete the service, data model, and data. To preserve data at a custom service, follow [these steps](https://v1.prisma.io/docs/1.34/faq/prisma-1-maintenace-fq12/#how-can-i-delete-custom-servers).**

### I'm not ready to upgrade to Prisma ORM. Can I self-host?

Yes, you can self-host. However, we recommend upgrading to [Prisma ORM](https://www.prisma.io/) as [Prisma 1](https://v1.prisma.io/docs) is in maintenance mode.
