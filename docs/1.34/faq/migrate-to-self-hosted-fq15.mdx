...
3. If your project uses a `PRISMA_MANAGEMENT_API_SECRET`, then you are **not** using Prisma 1 Cloud. You can verify this by using `prisma1 logout` from the command line and attempting to deploy or access a service.
...
4. Validate that the command works by using `prisma1 logout` in your console and then attempting to deploy or otherwise access your service using the newly declared environment variables.
