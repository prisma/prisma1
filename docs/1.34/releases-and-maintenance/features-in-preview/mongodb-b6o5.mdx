...
- `uri`: Your MongoDB [connection string](https://docs.mongodb.com/manual/reference/connection-string), i.e. the URL that can connect to your MongoDB server. Here's an overview of the information it needs to contain:
  ...
  - A boolean that specifies whether the connection to the MongoDB should use [SSL](https://docs.mongodb.com/manual/reference/connection-string/#urioption.ssl). (Default depends on the connection protocol. Default for `mongodb+srv`: `true`. Default for `mongodb`: `false`).
