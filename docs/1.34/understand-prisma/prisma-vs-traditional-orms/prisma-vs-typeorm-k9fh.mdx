...
This page is outdated! It compares **Prisma 1** with TypeORM. You can find an updated version comparing TypeORM with **Prisma 2** [here](https://www.prisma.io/docs/understand-prisma/api-comparisons/prisma-and-typeorm).
...
const userFragment = await prisma.user({ id }).$fragment(`
  fragment IdAndEmail on User { id email }`
`)
```
