...
Running [`prisma generate`](xcv2) in the directory where that `prisma.yml` is located generates a Prisma client in TypeScript and stores the generated files in a directory called `prisma-client`.
...
Here is an overview of the files that are generated for the TypeScript client:
