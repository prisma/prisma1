...
As mentioned above, Prisma's main use case is building GraphQL servers. The process of developing a GraphQL server commonly follows the model of _schema-driven development_ where a new API feature is first defined in the _GraphQL schema_ and then implemented using _resolver functions_.
