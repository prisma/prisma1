...
The data model of your service configuration has two major roles:

- **Define the underlying database schema** (they are mapped to database tables or equivalent structures, like _documents_, in the case of schemaless databases).
...
The data model is written in the GraphQL [Schema Definition Language](https://www.prisma.io/blog/graphql-sdl-schema-definition-language-6755bcb9ce51/) (SDL) and stored in one or more [`.graphql`-files](#files). These `.graphql`-files need to be referenced in your [prisma.yml](5cy7) under the `datamodel` property. For example:

<Code language="yml" lines="2">
...
- [Types](#object-types) consist of multiple [fields](#fields) and typically represent entities from your application domain (e.g. `User`, `Car`, `Order`). Each type in your data model is mapped to a database table (or an equivalent structure for schemaless databases) and CRUD operations are added to the GraphQL schema.
...
## Data model vs Prisma GraphQL schema
...
The Prisma GraphQL schema can be dowloaded from a Prisma service using the `get-schema` command of the [GraphQL CLI](https://github.com/graphql-cli/graphql-cli) or via an [introspection query](https://graphql.org/learn/introspection/) made directly against the service's API.
...
In real-world development setups, you want to keep your local Prisma GraphQL schema in sync with the service's API and ensure it gets updated upon every deploy of your Prisma service. That's what you can a use a _post-deployment_ [hook](5cy7#hooks-optional) for.
...
You typically don't redefine your model definitions in the application schema. Instead, you _import_ them using [`graphql-import`](https://github.com/prismagraphql/graphql-import):
...
You can write your data model in a single `.graphql`-file or split it accross multiple ones.
...
Every type you define in your data model will be available as an analogous type in the generated _Prisma GraphQL schema_.
...
The types in your data model affect the available operations in the Prisma API. Here is an overview of the generated CRUD and realtime operations for every type in your Prisma API: 
...
String values are currently limited to 256KB in size on Demo servers. This limit can be increased on other clusters using [the cluster configuration](https://github.com/graphcool/framework/issues/748).
...
When used as arguments in an operation, `DateTime` fields have to be specified in [ISO 8601 format](https://en.wikipedia.org/wiki/ISO_8601) with enclosing double quotes:

```graphql
query {
  user(birthday: "2015-11-22) { # November 22, 2015
    id
  }
}
```
...
When used as arguments in an operation, Enum fields have to be specified in without enclosing double quotes:
...
In a field definition, a type can be annotated with a _type modifier_. GraphQL supports two type modifies:
...
Fields can be configured with field constraints to add further semantics and enforce certain rules in your data model.
...
For example, considering the above data model, you can now retrieve a particular `User` node by its `email` address:
...
More database constraints will be added soon. Please join the discussion in this [feature request](https://github.com/prismagraphql/prisma/issues/728) if you have wish to see certain constraints implemented in Prisma.
...
The three fields `id`, `createdAt` and `updatedAt` have special semantics in Prisma. They are optional in your data model, but will always be maintained in the underlying database. This way you can always add the field to your data model later, and the data will still be available for existing nodes.

The values of these fields are currently read-only in the Prisma API (except when importing data) but will be made configurable in the future. See [this proposal](https://github.com/prismagraphql/prisma/issues/1278) for more information.
...
Fields in the data model affect the available [query arguments](tbd).
...
A more convenient workflow is discussed [in this feature request](https://github.com/prismagraphql/prisma/issues/2323) on Github.
...
Note that in this case the relation needs to be annotated with the [`@relation` directive](the-relation-directive).
...
When defining relations between types, you _can_ the `@relation` directive which provides meta-information about the relation. If a relation is ambiguous, you _must_ use the `@relation` directive to disambiguate it.
...
To learn more about when and how interfaces are coming to Prisma, check out this [feature request](https://github.com/prismagraphql/prisma/issues/83).
...
To learn more about when and how union types are coming to Prisma, check out this [feature request](https://github.com/prismagraphql/prisma/issues/165).