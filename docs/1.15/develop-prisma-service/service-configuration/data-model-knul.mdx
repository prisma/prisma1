...
You typically don't redefine your model definitions in the application schema. Instead, you _import_ them using [`graphql-import`](https://github.com/prisma/graphql-import):
...
An `ID` value is a generated unique 25-character string based on [cuid](https://github.com/prisma/cuid-java). Fields with `ID` values are [system fields](#system-fields) and just used internally, therefore it is not possible to create new fields with the `ID` type.
