...
You can find the latest beta image on [Docker Hub](https://hub.docker.com/r/prismagraphql/prisma/tags/) or simply use the `__VERSION__-alpha` tag when using the Docker CLI (where `__VERSION__` is a placeholder for the Prisma version you're targeting):

```bash
docker pull prismagraphql/prisma:1.21-alpha
```