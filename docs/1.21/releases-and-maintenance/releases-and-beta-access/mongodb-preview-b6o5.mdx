...
This means Prisma will never delete _or_ rename an existing collection from a database, nor will it drop an entire database. Be aware that this approach leads to "dangling collections", i.e. collections that still exist in the underlying MongoDB database but that can't be accessed through the Prisma API.
