export const meta = {
  title: 'MongoDB Preview (1.21-beta)',
  position: 200
}

## Concepts

With the `1.21-beta`-release, Prisma introduces a database connector for [MongoDB](https://www.mongodb.com/). MongoDB is a [document database](https://en.wikipedia.org/wiki/Document-oriented_database). Document databases stand in contrast to [relational databases](https://en.wikipedia.org/wiki/Relational_database) (RDBMS). 
...
MongoDB is a _schemaless_ database, which means it is _possible_ to insert data of various structures into it. MongoDB will never complain because some piece of inserted data doesn't adhere to some predefined format. This is different from relational databases where data that's inserted needs to adhere to the predefined [database schema](https://en.wikipedia.org/wiki/Database_schema).
...
- ... **directly**, it can not be guaranteed that the data that's stored in the database always adheres to the schema. In that case, you are responsible to ensure that the data that's written adheres to the schema.
...
- The name of the **MongoDB [database](https://docs.mongodb.com/manual/core/databases-and-collections/#databases) where the admin user is stored**. (Default: `admin`).
    ...
    environment:
      PRISMA_CONFIG: |
        ...
        databases:
          default:
            connector: mongo
            host: __MONGO_DB_HOST__
            port: __MONGO_DB_PORT__
            user: __MONGO_DB_ADMIN_USERNAME__
            password: __MONGO_DB_ADMIN_PASSWORD__
            authSource: __MONGO_DB_ADMIN_DATABASE_NAME__
            database: __MONGO_DB_DATABASE_NAME__
            rawAccess: false
            ssl: __USE_SSL__
```

Replace the seven placeholders that are written in uppercase letters with the appropriate values, e.g.:
...
> If you're running MongoDB on your local machine (without Docker but just by starting it manually using `mongod`), you need to set the `host` to `host.docker.internal` instead of `localhost`/`127.0.0.1`.
...
> **WARNING**: The `prisma introspect` can currently not be used to suggest a datamodel. Leave a +1 on [this](https://github.com/prisma/prisma/issues/3529) feature request if you're interested in this feature.
...
When using MongoDB with Prisma following the setup in the [previous section](#get-started), you can connect to the Mongo database directly using the [Mongo shell](https://docs.mongodb.com/manual/mongo/).
...
- `host`: The IP of the host machine that's running the MongoDB server `mongod`, e.g. `localhost`.
- `port`: The port where `mongod` is running on the host machine. ([Default](https://docs.mongodb.com/manual/reference/default-mongodb-port/): `27017`).
- `user` & `password`: The credentials for a MongoDB [admin user](https://docs.mongodb.com/manual/tutorial/enable-authentication/#user-administrator).
  - When starting with a **new database**, Prisma will create this user.
  - When starting with an **existing database**, Prisma looks up the user with these credentials in the `authSource` database (see next property).
- `authSource`: The  MongoDB [database](https://docs.mongodb.com/manual/core/databases-and-collections/#databases) where the admin user is stored. (Default: `admin`).
- `protocol`: Specifies whether you want to use the [standard connection format](https://docs.mongodb.com/manual/reference/connection-string/#standard-connection-string-formats)  (`mongodb`) or the [DNS Seedlist Connection Format](https://docs.mongodb.com/manual/reference/connection-string/#dns-seedlist-connection-format) (`mongodb+srv`). (Default: `mongodb`).
...
- `ssl`: A boolean that specifies whether the connection to the MongoDB should use [SSL](https://docs.mongodb.com/manual/reference/connection-string/#urioption.ssl). (Default: `false`) 