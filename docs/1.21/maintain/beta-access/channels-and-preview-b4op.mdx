...
Some Prisma features are releases in a **Preview** mode. The Preview mode is orthogonal to the release channels, meaning a feature that's released in Preview mode still goes through the "normal" release stages: `alpha`, `beta` and `stable`.

Preview mode means that the API is of the feature is final and, but there might be significant internal improvements as long as the feature is in Preview mode.
