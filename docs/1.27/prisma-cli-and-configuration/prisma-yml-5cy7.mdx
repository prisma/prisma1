...
The `datamodel` points to one or more `.prisma`-files containing model definitions written in GraphQL SDL. If multiple files are provided, the CLI simply concatenates their contents at deployment time.
