...
When you're updating the Prisma datamodel and run `prisma deploy` to apply the changes, Prisma will only ever make _additive_ structural changes to your underlying MongoDB database(s).
...
You can use [`prisma introspect`](https://github.com/prisma/prisma/issues/3529) to suggest a datamodel.
