...
Before executing the code, let's add a `start` script to `package.json` so you can comfortably run the code. Adjust your `package.json` to include a `start` script:
