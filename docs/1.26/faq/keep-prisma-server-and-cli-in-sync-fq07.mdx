...
export const meta = {
  ...
  position: 5,
  articleGroup: 'Keep Prisma server & CLI in sync',
}
...
If you have access to the Docker file that was used to deploy your Prisma server, you can find the Prisma server version under the `services.prisma.image` property. For example, the following Docker file deploys a Prisma server running on version `1.26`:
