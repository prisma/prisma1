...
You can ensure that the CRUD building blocks are automatically being updated after every deploy by also adding the `nexus-prisma-generate` command as a `post-deploy` hook in your `prisma.yml`:
...
Before you start with the implementation, go ahead and start your development server to generate TS definitions:
