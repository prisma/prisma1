...
To be able to specify an `endpoint`, you need to have access to a Prisma server. If you don't have access to a Prisma server, you can omit the `endpoint` from prisma.yml and run `prisma1 deploy` only with the `datamodel` property. In that case, a CLI wizard guides you through the process of creating a local Prisma server or lets you deploy to a Demo server in Prisma Cloud. It then writes the `endpoint` into prisma.yml for you before the service gets deployed:
...
hooks:
  post-deploy:
    - graphql get-schema --endpoint http://localhost:4466/myservice/dev --output prisma.graphql
```
...
# A "post-deployment" hook that first downloads the GraphQL
# schema from an endpoint configured in `.graphqlconfig` and
# then invokes a code generation process.
hooks:
  post-deploy:
...
The data model is defined in two files called `types.graphql` and `enums.graphl`. When the service gets deployed, the contents of both files will be concatenated by the CLI.
...
1. Download the GraphQL schema for the `db` project specified in `.graphqlconfig`
1. Invoke code generation as specified in `.graphqlconfig`
...
Seeds are implicitly executed when deploying a service for the first time (unless explicitly disabled using the `--no-seed` flag on `prisma1 deploy`). Track [this feature request for additional seeding workflows](https://github.com/graphcool/prisma/issues/1536).
