...
The datamodel is written in the GraphQL [Schema Definition Language](https://www.prisma.io/blog/graphql-sdl-schema-definition-language-6755bcb9ce51/) (SDL) and stored in one or more [`.graphql`-files](#files). These `.graphql`-files need to be referenced in your [prisma.yml](5cy7) under the `datamodel` property. For example:
...
- [Types](#object-types) consist of multiple [fields](#fields) and typically represent entities from your application domain (e.g. `User`, `Car`, `Order`). Each type in your datamodel is mapped to a database table (or an equivalent structure for schemaless databases) and CRUD operations are added to the GraphQL schema.
...
- The **datamodel** containing the _model definitions_ for your service's APIs, typically called `datamodel.prisma`.
...
Fields in the datamodel affect the available [query arguments](tbd).
