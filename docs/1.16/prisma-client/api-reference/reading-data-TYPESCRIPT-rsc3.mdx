...
- You can view what the generated TypeScript client [here](https://github.com/nikolasburk/prisma-client-examples/blob/master/generated/prisma.ts).
...
```ts
const user: UserNode = await prisma.user({ email: "alice@prisma.io" })
```
...
The second argument on each generated query function is an optional _fragment_ that allows to specify exactly which fields of the requested model should be included in the result.
...
const userWithPosts = await prisma.users({}, fragment)
```
...
- View the type of `orderBy` for `Link` [Vote](https://github.com/nikolasburk/prisma-client-examples/blob/master/generated/prisma.ts#L125)
