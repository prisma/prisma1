export const meta = {
  title: 'Building GraphQL Servers with Prisma Bindings',
  position: 40,
  description: 'On the previous pages, you learned about the relevant pieces of Prisma services and how to configure their APIs. This page now explains Prisma\'s most common use case: Building GraphQL servers with Prisma bindings and Prisma as the data access layer.'
}
...
For each type in your datamodel, the `Prisma` binding creates a dedicated `exists` function that is named after the type. Here are a few examples demonstrating how `exists` can be used:
...
## A layered architecture for your GraphQL server: Two GraphQL APIs
...
1. **Configure database layer**
    ...
    1. _Deploy_ a Prisma service based on that datamodel
...
#### 1.1. Specify datamodel for Prisma service

The first step is to define the datamodel for a Prisma service, here is the one for this example:
...
To deploy the Prisma service, we also need a minimal prisma.yml. Assumed the above datamodel is stored in a file called `datamodel.prisma`, this is what prisma.yml could look like:
...
The client that initiated the request will receive this JSON object in the HTTP response sent by the server.
