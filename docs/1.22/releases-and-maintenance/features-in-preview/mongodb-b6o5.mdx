...
Therefore, when starting out with an existing MongoDB, it is extremely important that the legacy data is consistent and that it can be modeled accurately as a Prisma datamodel. You can use the `prisma1 introspect` command to derive a datamodel from the data that's currently stored in your MongoDB. `prisma1 introspect` is currently available in the [beta version](b5op) of the Prisma CLI.
...
You can use `prisma1 introspect` to suggest a datamodel. It is available in the current [beta](b5op) version of the Prisma CLI.
