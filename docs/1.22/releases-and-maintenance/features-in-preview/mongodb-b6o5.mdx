...
With the `1.22`-release, Prisma introduces a database connector for [MongoDB](https://www.mongodb.com/). MongoDB is a [document database](https://en.wikipedia.org/wiki/Document-oriented_database). Document databases stand in contrast to [relational databases](https://en.wikipedia.org/wiki/Relational_database) (RDBMS). 
...
fc261a149740        prismagraphql/prisma:1.22    "/bin/sh -c /app/sta…"   About a minute ago   Up About a minute   0.0.0.0:4466->4466/tcp     hello-mongo_prisma_1
```
...
Therefore, when starting out with an existing MongoDB, it is extremely important that the legacy data is consistent and that it can be modeled accurately as a Prisma datamodel. You can use the `prisma introspect` command to derive a datamodel from the data that's currently stored in your MongoDB. `prisma introspect` is currently available in the [beta version](b5op) of the Prisma CLI.
...
You can use `prisma introspect` to suggest a datamodel. It is available in the current [beta](b5op) version of the Prisma CLI.
