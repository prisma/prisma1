...
class ProjectJsonFormatterSpec extends FlatSpec with Matchers with JsonUtils {
  ...
  "Index Fields" should "be readable in the obsolete format with no indexes defined" in {
    val json =
      """
        |{
        |  "name": "A",
        |  "stableIdentifier": "cjpijtqs50015le78e34hbb2s",
        |  "isEmbedded": false,
        |  "fields": [
        |    {
        |      "name": "id",
    ...
    val result = json.as[ModelTemplate]
    result.indexes should be(empty)
  }

  "Index Fields" should "be readable if they are defined" in {
    val json =
      """
        |{
        |  "name": "A",
    ...
    val result = json.as[ModelTemplate]
    result.indexes.size should be(1)
    result.indexes should contain(Index(fields = Vector("herp", "derp"), name = "herp_derp_idx"))
  }
