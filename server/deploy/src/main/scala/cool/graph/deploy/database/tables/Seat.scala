...
class SeatTable(tag: Tag) extends Table[Seat](tag, "Seat") {
  ...
  def project = foreignKey("seat_projectid_foreign", projectId, Tables.Projects)(_.id)

  def clientId = column[Option[String]]("clientId")
  def client = foreignKey("seat_clientid_foreign", clientId, Tables.Clients)(_.id.?)

  def * = (id, status, email, projectId, clientId) <> ((Seat.apply _).tupled, Seat.unapply)
}
