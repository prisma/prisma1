...
object UserInputErrors {
  ...
  case class ResolverPayloadIsRequired(resolverName: String) extends SystemApiError(s"The payloadType for the resolver `$resolverName` is not nullable, but the resolver returned null.", 2063)
...
object RequestPipelineErrors {
  ...
  case class DataDoesNotMatchPayloadType(functionName: String) extends RequestPipelineError(s"""The value of the data object did not match the specified payloadType for function: $functionName""", 5007)
