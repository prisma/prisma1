...
import com.prisma.messagebus.ConsumerRef
...
case class RabbitQueue[T](
  ...
  val exchange: rabbit.Exchange          = RabbitUtils.declareExchange(amqpUri, exchangeName, exchangeConcurrency, durableExchange)
...
case class RabbitQueuesRef(mainQ: rabbit.Queue, errorQ: rabbit.Queue)
