    ...
    def loop(node: AstNode): AstNode =
      node match {
        ...
        case n @ NamedType(_, _) ⇒
          ...
          map(cmd, n)
        case n @ StringValue(_, _, _, comments, _) ⇒
          val cmd = onEnter(n)
          if (breakOrSkip(cmd)) {
            comments.foreach(s ⇒ loop(s))
            breakOrSkip(onLeave(n))
          }
          map(cmd, n)
        ...
        case n @ ScalarTypeDefinition(_, dirs, _, comment, _) ⇒
          if (breakOrSkip(onEnter(n))) {
          ...
          n
        case n @ FieldDefinition(name, fieldType, args, dirs, _, comment, _) ⇒
          if (breakOrSkip(onEnter(n))) {
            loop(fieldType)
          ...
          n
        case n @ InputValueDefinition(_, valueType, default, dirs, _, comment, _) ⇒
          if (breakOrSkip(onEnter(n))) {
            loop(valueType)
            default.foreach(d ⇒ loop(d))
            dirs.foreach(d ⇒ loop(d))
          ...
          n
        case n @ ObjectTypeDefinition(_, interfaces, fields, dirs, _, comment, trailingComments, _) ⇒
          if (breakOrSkip(onEnter(n))) {
          ...
          n
        case n @ InterfaceTypeDefinition(_, fields, dirs, comment, _, trailingComments, _) ⇒
          if (breakOrSkip(onEnter(n))) {
            fields.foreach(d ⇒ loop(d))
            dirs.foreach(d ⇒ loop(d))
            comment.foreach(s ⇒ loop(s))
          ...
          n
        case n @ UnionTypeDefinition(_, types, dirs, _, comment, _) ⇒
          if (breakOrSkip(onEnter(n))) {
          ...
          n
        case n @ EnumTypeDefinition(_, values, dirs, comment, _, trailingComments, _) ⇒
          if (breakOrSkip(onEnter(n))) {
          ...
          n
        case n @ EnumValueDefinition(_, dirs, _, comment, _) ⇒
          if (breakOrSkip(onEnter(n))) {
            dirs.foreach(d ⇒ loop(d))
            comment.foreach(s ⇒ loop(s))
            breakOrSkip(onLeave(n))
        ...
        case n @ TypeExtensionDefinition(definition, comment, _) ⇒
          ...
          n
        case n @ DirectiveDefinition(_, args, locations, _, comment, _) ⇒
          if (breakOrSkip(onEnter(n))) {
            args.foreach(d ⇒ loop(d))
