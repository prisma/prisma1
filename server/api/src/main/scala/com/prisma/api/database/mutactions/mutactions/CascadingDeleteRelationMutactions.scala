...
import com.prisma.shared.models.{Field, Project}
...
case class CascadingDeleteRelationMutactions(project: Project, path: Path) extends ClientSqlDataChangeMutaction {
  ...
  val removalActions = relationsWhereThisIsNotRequired.map(relation => DatabaseMutationBuilder.deleteRelationRowsByPath(project.id, relation, path))

  val deleteAction = List(DatabaseMutationBuilder.deleteDataItemsByPath(project.id, path))

  override def execute = {
    val allActions = requiredCheck ++ removalActions ++ deleteAction
