...
case class UpsertDataItem(
  ...
  override def handleErrors = {
    ...
    Some({
      ...
      case e: SQLIntegrityConstraintViolationException if e.getErrorCode == 1062 => APIErrors.UniqueConstraintViolation(model.name, getFieldFromCoolArgs(List(createArgs, updateArgs), e))
