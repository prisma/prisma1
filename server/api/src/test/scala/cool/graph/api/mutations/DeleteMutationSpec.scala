...
class DeleteMutationSpec extends FlatSpec with Matchers with ApiBaseSpec {
  ...
  "A Delete Mutation" should "gracefully fail on non-existing id" in {
    val id = server.executeQuerySimple(s"""mutation {createScalarModel(data: {string: "test"}){id}}""", project = project).pathAsString("data.createScalarModel.id")
    server.executeQuerySimpleThatMustFail(s"""mutation {deleteScalarModel(where: {id: "DOES NOT EXIST"}){id}}""", project = project, errorCode =3002, errorContains = s"""'ScalarModel' has no item with id 'DOES NOT EXIST'""")
  ...
  "A Delete Mutation" should "delete and return item on non id unique field" in {
    ...
    server.executeQuerySimple(s"""mutation {deleteScalarModel(where: {unicorn: "a"}){unicorn}}""", project = project, dataContains = s"""{"deleteScalarModel":{"unicorn":"a"}""")
  ...
  "A Delete Mutation" should "gracefully fail when trying to delete on non-existent value for non id unique field" in {
    server.executeQuerySimple(s"""mutation {createScalarModel(data: {unicorn: "a"}){id}}""", project = project)
    server.executeQuerySimpleThatMustFail(s"""mutation {deleteScalarModel(where: {unicorn: "c"}){unicorn}}""", project = project, errorCode = 3002, errorContains = "'ScalarModel' has no item with unicorn 'c'")
