...
class GCDBValueConverterSpec extends FlatSpec with Matchers {
  ...
  "It should take list GCValues and" should "convert them to DBValues and back without loss if the type and list status are correct." in {
  ...
  def forthAndBack(input: GCValue, typeIdentifier: TypeIdentifier, isList: Boolean) = {
    val converter    = GCDBValueConverter(typeIdentifier, isList)
