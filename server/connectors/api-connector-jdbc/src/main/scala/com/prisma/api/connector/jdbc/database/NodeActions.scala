...
trait NodeActions extends BuilderBase with FilterConditionBuilder with ScalarListActions with RelayIdActions {
  ...
  def createNode(model: Model, args: PrismaArgs): DBIO[IdGCValue] = {
    ...
    val argsWithIdIfNecessary = if (idIsAutoGeneratedByDb) { args.rootGC } else {
      if (args.rootGCMap.get(model.idField_!.name).isDefined && !args.rootGCMap.get(model.idField_!.name).contains(NullGCValue)) { args.rootGC } else {
        args.rootGC.add(model.idField_!.name, generateId(model))
      }
    }
