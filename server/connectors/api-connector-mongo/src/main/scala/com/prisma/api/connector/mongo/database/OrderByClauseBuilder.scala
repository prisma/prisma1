...
object OrderByClauseBuilder {
  ...
  def sortStage(args: QueryArguments): conversions.Bson = {
    ...
    //the secondary field is always ascending no matter what the primary field says. If we need to revert due to last being defined it is always descending.
    (orderByArg, defaultOrder, isReverseOrder) match {
      case (Some(order), "asc", true) if order.field.dbName != idField   => sort(orderBy(descending(order.field.dbName), descending(idField)))
      case (Some(order), "desc", true) if order.field.dbName != idField  => sort(orderBy(ascending(order.field.dbName), descending(idField)))
      case (Some(order), "asc", false) if order.field.dbName != idField  => sort(orderBy(ascending(order.field.dbName), ascending(idField)))
      case (Some(order), "desc", false) if order.field.dbName != idField => sort(orderBy(descending(order.field.dbName), ascending(idField)))
      case (_, "asc", true)                                              => sort(descending(idField))
      case (_, "desc", true)                                             => sort(ascending(idField))
      case (_, "asc", false)                                             => sort(ascending(idField))
      case (_, "desc", false)                                            => sort(descending(idField))
      case x                                                             => sys.error(s"$x is unhandled in this pattern match")
    }
  }
}
