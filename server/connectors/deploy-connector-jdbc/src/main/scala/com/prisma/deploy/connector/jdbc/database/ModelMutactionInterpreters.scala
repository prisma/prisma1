...
import com.prisma.deploy.connector.{CreateModelTable, DeleteModelTable, RenameTable, Tables}
...
case class CreateModelInterpreter(builder: JdbcDeployDatabaseMutationBuilder) extends SqlMutactionInterpreter[CreateModelTable] {
  ...
  override def execute(mutaction: CreateModelTable, schemaBeforeMigration: Tables): DBIOAction[Any, NoStream, Effect.All] = {
  ...
  override def rollback(mutaction: CreateModelTable, schemaBeforeMigration: Tables) =
...
case class DeleteModelInterpreter(builder: JdbcDeployDatabaseMutationBuilder) extends SqlMutactionInterpreter[DeleteModelTable] {
  ...
  override def execute(mutaction: DeleteModelTable, schemaBeforeMigration: Tables) = {
  ...
  override def rollback(mutaction: DeleteModelTable, schemaBeforeMigration: Tables) = builder.createModelTable(
    projectId = mutaction.projectId,
    model = mutaction.model
  )
}
...
case class RenameModelInterpreter(builder: JdbcDeployDatabaseMutationBuilder) extends SqlMutactionInterpreter[RenameTable] {
  override def execute(mutaction: RenameTable, schemaBeforeMigration: Tables)  = setName(mutaction, mutaction.previousName, mutaction.nextName)
  override def rollback(mutaction: RenameTable, schemaBeforeMigration: Tables) = setName(mutaction, mutaction.nextName, mutaction.previousName)
