...
object ConnectorUtils {
  def loadApiConnector(config: PrismaConfig, isTest: Boolean = false)(implicit ec: ExecutionContext): ApiConnector = {
    ...
    (databaseConfig.connector, databaseConfig.active) match {
      ...
      case ("mongo", _)           => MongoApiConnector(databaseConfig, isTest = isTest)
