...
object AnyMutactionInterpreterImpl extends SqlMutactionInterpreter[DeployMutaction] {
  override def execute(mutaction: DeployMutaction) = {
    mutaction match {
      ...
      case x: CreateInlineRelationForTests => CreateInlineRelationInterpreter.execute(x)
    }
  }
  ...
  override def rollback(mutaction: DeployMutaction) = {
    mutaction match {
      ...
      case x: CreateInlineRelationForTests => CreateInlineRelationInterpreter.rollback(x)
    }
  }
}
