...
import com.prisma.deploy.migration.inference.GenericProblem
...
case class InferredTablesValidator() {

  def checkRelationsAgainstInferredTables(schema: Schema, inferredTables: InferredTables): Seq[GenericProblem] = {
      ...
      relation.manifestation match {
        ...
        case Some(m: InlineRelationManifestation) =>
          ...
          inferredTables.modelTables.find(_.name == model.dbName) match {
            case None =>
              Some(GenericProblem(s"Could not find the model table ${model.dbName} in the database"))
