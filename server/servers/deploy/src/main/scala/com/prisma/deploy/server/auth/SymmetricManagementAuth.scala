...
import com.prisma.deploy.schema.{AuthFailure, TokenExpired}
...
case class SymmetricManagementAuth(jwtSecret: String) extends ManagementAuth {
  ...
  override def verify(name: String, stage: String, authHeaderOpt: Option[String]): Try[Unit] = Try {
    authHeaderOpt match {
      case None =>
        throw AuthFailure("'Authorization' header not provided")
      ...
      case Some(authHeader) =>
        ...
        decodedToken match {
          case Failure(exception) =>
            throw AuthFailure(s"Token can't be decoded: ${exception.getMessage}")
          ...
          case Success(rawToken) =>
            ...
            token.grants
              ...
              .getOrElse(throw AuthFailure(s"Token contained ${token.grants.length} grants but none satisfied the request."))
        }
    }
  }
  ...
  private def verifyGrant(nameToCheck: String, stageToCheck: String, grant: TokenGrant): Boolean = {
    val (grantedName: String, grantedStage: String) = grant.target.split("/").toVector match {
      ...
      case invalid                => throw AuthFailure(s"Contained invalid grant '$invalid'")
    }

    if (grantedName == "" || grantedStage == "") {
      throw AuthFailure(s"Both service and stage must be defined in grant '$grant'")
    }
  ...
  private def parseToken(token: String): TokenData = {
    Json.parse(token).asOpt[TokenData] match {
      case None              => throw AuthFailure(s"Failed to parse token data")
