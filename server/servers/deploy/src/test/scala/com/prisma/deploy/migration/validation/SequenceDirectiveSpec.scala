...
import com.prisma.shared.models.ConnectorCapability.IdSequenceCapability
...
class SequenceDirectiveSpec extends WordSpecLike with Matchers with DataModelValidationSpecBase {
  "should be parsed correctly" in {
    val dataModelString =
      """
        |type Model {
        |  id: ID! @id(strategy: SEQUENCE) @sequence(name: "MY_SEQUENCE" initialValue:11 allocationSize:123)
        |}
      """.stripMargin

    val dataModel           = validate(dataModelString, Set(IdSequenceCapability))
  ...
  "should error if it is used on fields that are not marked as id" in {
    ...
    error.description should equal("The directive `@sequence` must only be specified for fields that are marked with `@id(strategy: SEQUENCE)`.")
  }
