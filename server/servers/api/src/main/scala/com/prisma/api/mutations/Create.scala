...
//    val sssActions             = ServerSideSubscriptions.extractFromMutactions(project, createMutactionsResult, requestId)
    ...
    Future.successful {
      PreparedMutactions(
        ...
        sideEffectMutactions = Vector.empty // sssActions ++ subscriptionMutactions
      )
    }
  }

  override def getReturnValue(results: MutationResult): Future[ReturnValueResult] = {
    val createdItem = results.mutactionResults.collectFirst { case x: CreateDataItemResult => x }
    for {
      returnValue <- returnValueByUnique(NodeSelector.forIdGCValue(model, createdItem.map(_.id).get))
