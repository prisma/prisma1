...
class BulkImport(project: Project)(implicit apiDependencies: ApiDependencies) extends PlaySprayConversions {
  ...
  def executeImport(json: JsValue): Future[JsValue] = {
    ...
    val mutactions: Vector[DatabaseMutaction Or Exception] =
      bundle.valueType match {
        case "nodes"     => Vector(Good(generateImportNodesDBActions(bundle.values.elements.map(convertToImportNode))))
  ...
  private def generateImportNodesDBActions(nodes: Vector[ImportNode]): CreateDataItemsImport = {
    require(nodes.nonEmpty)
    val args = nodes.map(node => ReallyCoolArgs(node.values))
    val node = nodes.head
    CreateDataItemsImport(project, node.model, args)
  }
