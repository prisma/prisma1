...
class BulkExportSpec extends FlatSpec with Matchers with ApiSpecBase with AwaitUtils {
  ...
  // This case is duplicated. Find the spec for the Postgres connector below.
  // The ordering of results between MySQL and Postgres differ slightly because the Postgres connector had to be changed for enabling the passive style.
  "Exporting relationData with MySQL" should "work (filesizelimit set to 1000)" taggedAs (IgnoreMySql) in {
    val nodes =
      """{ "valueType": "nodes", "values": [
  ...
  // This case is duplicated. Find the spec for the Postgres connector below.
  // The ordering of results between MySQL and Postgres differ slightly because the Postgres connector had to be changed for enabling the passive style.
  "Exporting relationData with Postgres" should "work (filesizelimit set to 1000)" taggedAs (IgnorePostgres) in {
    val nodes =
      """{ "valueType": "nodes", "values": [
        |{"_typeName": "Model0", "id": "0", "a": "test", "b":  0, "createdAt": "2017-11-29 14:35:13"},
        |{"_typeName": "Model1", "id": "1", "a": "test", "b":  1},
