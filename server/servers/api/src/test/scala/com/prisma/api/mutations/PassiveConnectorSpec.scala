...
class PassiveConnectorSpecForInlineRelations extends PassiveConnectorSpec {
  ...
  lazy val inlineRelationProject = SchemaDsl.fromPassiveConnectorSdl(id = projectId, testDependencies.deployConnector, withReservedFields = false) {
    """
  ...
  "the connector" should "support diverging names for models/tables and fields/columns" taggedAs (IgnoreActive) in {
    executeOnInternalDatabase(inlineRelationSchema)
    val project = SchemaDsl.fromPassiveConnectorSdl(id = projectId, testDependencies.deployConnector, withReservedFields = false) {
      """
        | type List @model(table: "list"){
        |   id: ID!
...
class PassiveConnectorSpecForTableRelations extends FlatSpec with PassiveConnectorSpec with Matchers with ApiSpecBase {
  ...
  lazy val relationTableProject = SchemaDsl.fromPassiveConnectorSdl(id = projectId, testDependencies.deployConnector, withReservedFields = false) {
    """
      | type List @model(table: "list"){
      |   id: ID!
      |   name: String!
...
class PassiveConnectorSpecForAutoGeneratedIds extends FlatSpec with PassiveConnectorSpec with Matchers with ApiSpecBase {
  ...
  lazy val project = SchemaDsl.fromPassiveConnectorSdl(id = projectId, testDependencies.deployConnector, withReservedFields = false) {
    """
      | type List @model(table: "list"){
      |   id: Int!
      |   name: String!
