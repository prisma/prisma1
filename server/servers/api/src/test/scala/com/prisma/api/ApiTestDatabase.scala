...
case class ApiTestDatabase()(implicit dependencies: TestApiDependencies) extends AwaitUtils {
  ...
  def deleteProjectDatabase(project: Project): Unit   = dependencies.deployConnector.deleteProjectDatabase(project.id)
  private def createProjectDatabase(project: Project) = dependencies.deployConnector.createProjectDatabase(project.id)
