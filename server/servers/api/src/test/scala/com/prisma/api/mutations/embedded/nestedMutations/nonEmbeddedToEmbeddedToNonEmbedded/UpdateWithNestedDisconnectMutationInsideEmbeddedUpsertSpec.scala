...
class UpdateWithNestedDisconnectMutationInsideEmbeddedUpsertSpec extends FlatSpec with Matchers with ApiSpecBase with SchemaBaseV11 {
    ...
    val update = server
      .queryThatMustFail(
        s"""mutation {
          ...
          |       update:{ friendReq:{disconnect:true}}
          |    }}
          |  }){
          |    p
          |    children{
        ...
        0,
        errorContains =
          """Reason: 'children.upsert[0].update.friendReq.disconnect' Field 'disconnect' is not defined in the input type 'FriendUpdateOneRequiredInput'."""
      )
    ...
    val update = server
      .query(
        s"""mutation {
          ...
          |       update:{ friendOpt:{disconnect:true}}
          |    }}
          |  }){
          |    p
          |    children{
    ...
    val update = server
      .query(
        s"""mutation {
          ...
          |       update:{ friendsOpt:{disconnect:{f:"f1"}}}
          |    }}
          |  }){
          |    p
          |    children{
