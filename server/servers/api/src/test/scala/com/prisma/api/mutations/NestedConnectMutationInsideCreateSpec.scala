...
class NestedConnectMutationInsideCreateSpec extends FlatSpec with Matchers with ApiBaseSpec {
  ...
  "a PM to CM  relation with the children already in a relation" should "be connectable through a nested mutation by unique" in {
    ...
    database.runDbActionOnClientDb(ApiDatabaseQueryBuilderPostGres.itemCountForTable(project.id, "_ChildToParent")) should be(Vector(4))
  }
  ...
  "a PM to CM  relation without a backrelation" should "be connectable through a nested mutation by unique" in {
    ...
    database.runDbActionOnClientDb(ApiDatabaseQueryBuilderPostGres.itemCountForTable(project.id, "_UserToRole")) should be(Vector(0))
    ...
    database.runDbActionOnClientDb(ApiDatabaseQueryBuilderPostGres.itemCountForTable(project.id, "_UserToRole")) should be(Vector(1))
  }
