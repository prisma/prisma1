...
class CascadingDeleteSpec extends FlatSpec with Matchers with ApiBaseSpec {
  ...
  "P1!-C1! relation deleting the parent marked cascading" should "error if the child is required in another non-cascading relation" in {
    ...
    database.runDbActionOnClientDb(ApiDatabaseQueryBuilderPostGres.itemCountForTable(project.id, "_RelayId")) should be(Vector(6))
  }
  ...
  "A path that is interrupted since there are nodes missing" should "only cascade up until the gap" in {
    ...
    database.runDbActionOnClientDb(ApiDatabaseQueryBuilderPostGres.itemCountForTable(project.id, "_RelayId")) should be(Vector(5))
  }
  ...
  "A deep uninterrupted path" should "error on a required relation violation at the end" in {
    ...
    database.runDbActionOnClientDb(ApiDatabaseQueryBuilderPostGres.itemCountForTable(project.id, "_RelayId")) should be(Vector(9))
  }
  ...
  "A required relation violation on the parent" should "roll back all cascading deletes on the path" in {
    ...
    database.runDbActionOnClientDb(ApiDatabaseQueryBuilderPostGres.itemCountForTable(project.id, "_RelayId")) should be(Vector(7))
  }
