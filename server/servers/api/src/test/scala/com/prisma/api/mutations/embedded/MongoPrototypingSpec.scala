  ...
  "UpdateMany" should "work" in {
    ...
    val create = server.query(
      s"""mutation {
         |   createParent(data: {
         |   name: "Dad",
         |   children: {create:[{ name: "Daughter"}, { name: "Daughter2"}]}
         |}){
         |  name,
         |  children{ name}
         |}}""",
    ...
    create.toString should be("""{"data":{"createParent":{"name":"Dad","children":[{"name":"Daughter"},{"name":"Daughter2"}]}}}""")

    val updateMany = server.query(
      s"""mutation {
         |   updateParent(
         |   where: { name: "Dad" }
