...
class CascadingDeletePathSpec extends FlatSpec with Matchers with ApiSpecBase {
  ...
  "Paths" should "not be generated for non-cascading relations" in {
    ...
    val res    = collectCascadingPaths(Path.empty(NodeSelector.forCuid(parent, "does not exist")))
  ...
  "Paths for graphs with  circles" should "error" in {
    ...
    val parent = project.schema.getModelByName_!("P")
    assertThrows[CascadingDeletePathLoops] { collectCascadingPaths(Path.empty(NodeSelector.forCuid(parent, "does not exist"))) }
  }
