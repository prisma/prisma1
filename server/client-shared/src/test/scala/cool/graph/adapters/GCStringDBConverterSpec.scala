...
class GCStringDBConverterSpec extends FlatSpec with Matchers {
  ...
  "It should take list GCValues and" should "convert them to DBString and back" in {
    println("ListValues")
...
//    lists
  ...
  def forthAndBack(input: GCValue, typeIdentifier: TypeIdentifier, isList: Boolean) = {
    val converter    = GCStringDBConverter(typeIdentifier, isList)
    val forth        = converter.fromGCValue(input)
    val forthAndBack = converter.toGCValueCanReadOldAndNewFormat(forth)
    println("Input: " + input + " Forth: " + forth + " Output: " + forthAndBack)

    forthAndBack match {
      case Good(x) =>
        println(forthAndBack.get)
        if (x == input) Result.Equal else Result.NotEqual
      case Bad(error) =>
