...
impl<'a> Renderer<'a, DMMFFieldWrapper> for DMMFFieldRenderer<'a> {
    fn render(&self, ctx: RenderContext) -> (DMMFFieldWrapper, RenderContext) {
        match &self {
            ...
            DMMFFieldRenderer::Output(output) => self.render_output_field(output, ctx),
        }
    }
}
...
impl<'a> DMMFFieldRenderer<'a> {
    ...
    fn render_output_field(&self, field: &Field, ctx: RenderContext) -> (DMMFFieldWrapper, RenderContext) {
