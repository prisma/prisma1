use super::Builder;
use crate::{CoreResult, Query as PrismaQuery, QuerySchemaRef, RootMutationBuilder};
...
impl RootBuilder {
    ...
    fn build_mutation(&self, root_selections: &Vec<Selection>) -> CoreResult<Vec<PrismaQuery>> {
        root_selections
            .iter()
            .map(|selection| match selection {
                Selection::Field(ref field) => RootMutationBuilder::new(Arc::clone(&self.query_schema), field)
                    .build()
