...
impl Ordering {
    pub fn for_model(model: &Model, order_by: &Option<OrderBy>, reverse: bool) -> PrismaResult<OrderVec> {
    ...
    fn by_fields(
        ...
        let ordering = match first_field {
            Some(ref first) if first.db_name() != second_field.db_name() => match (default_order, reverse) {
                (SortOrder::Asc, true) => vec![first.as_column().descend(), second_field.as_column().descend()],
                (SortOrder::Desc, true) => vec![first.as_column().ascend(), second_field.as_column().descend()],
                (SortOrder::Asc, false) => vec![first.as_column().ascend(), second_field.as_column().ascend()],
                (SortOrder::Desc, false) => vec![first.as_column().descend(), second_field.as_column().ascend()],
            },
