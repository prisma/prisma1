...
fn error_if_the_datamodel_is_invalid() {
    ...
    let result = handle_command(&json);
    assert_eq!(result, r#"{"jsonrpc":"2.0","error":{"code":4466,"message":"An error happened. Check the data field for details.","data":{"code":1000,"errors":["Error parsing attribute @default: Expected Int, but received String value Function<String>"],"type":"DataModelErrors"}},"id":1}"#);
}
