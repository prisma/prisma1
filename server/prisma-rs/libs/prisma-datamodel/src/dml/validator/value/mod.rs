...
impl ValueValidator for WrappedValue {
    fn as_str(&self) -> Result<String, ValueParserError> {
        match &self.value {
            ...
            _ => Err(ValueParserError::new(format!(
                "Expected String Value, received {:?}",
                self.value
            ))),
        }
    }
    ...
    fn as_int(&self) -> Result<i32, ValueParserError> {
        match &self.value {
            ...
            _ => Err(ValueParserError::new(format!(
                "Expected Numeric Value, received {:?}",
                self.value
            ))),
        }
    }
    ...
    fn as_bool(&self) -> Result<bool, ValueParserError> {
        match &self.value {
            ...
            _ => Err(ValueParserError::new(format!(
                "Expected Boolean Value, received {:?}",
                self.value
            ))),
        }
    }
    ...
    fn as_constant_literal(&self) -> Result<String, ValueParserError> {
        match &self.value {
            ...
            _ => Err(ValueParserError::new(format!(
                "Expected Constant Value, received {:?}",
                self.value
            ))),
        }
    }
...
impl ValueValidator for WrappedErrorValue {
    fn as_str(&self) -> Result<String, ValueParserError> {
        Err(ValueParserError::new(self.message.clone()))
    }
    fn as_int(&self) -> Result<i32, ValueParserError> {
        Err(ValueParserError::new(self.message.clone()))
    }
