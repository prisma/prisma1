use crate::{ast::{self, WithSpan}, errors::ErrorCollection, errors::ValidationError};
...
impl Precheck {
    ...
    pub fn precheck_generator_config<'a>(config: &'a ast::GeneratorConfig, errors: &mut ErrorCollection) {
        ...
        for arg in &config.properties {
            args.check_duplicate(&format!("generator configuration {}", config.name), &arg.name, &arg, errors);
        }
    }
    ...
    pub fn precheck_source_config<'a>(config: &'a ast::SourceConfig, errors: &mut ErrorCollection) {
        let mut args = HashMap::<&'a str, &'a ast::Argument>::new();
        for arg in &config.properties {
            args.check_duplicate(&format!("datasource configuration {}", config.name), &arg.name, &arg, errors);
        }
    }
}
...
impl<'a> CheckDuplicate<&'a str, &'a ast::Top> for HashMap<&'a str, &'a ast::Top>
{
    fn check_duplicate(&mut self, _parent_name: &str, key: &'a str, obj: &'a ast::Top, errors: &mut ErrorCollection) {
        if self.contains_key(key) {
            errors.push(ValidationError::new_duplicate_top_error(obj.get_type(), self[key].get_type(), key, obj.span()));
        } else {
            self.insert(key, obj);
        }
    }
...
{
    fn check_duplicate(&mut self, parent_name: &str, key: &'a str, value: &'a ast::EnumValue, errors: &mut ErrorCollection) {
        if self.contains_key(key) {
            errors.push(ValidationError::new_duplicate_enum_value_error(parent_name, key, &value.span));
        } else {
...
{
    fn check_duplicate(&mut self, parent_name: &str, key: &'a str, field: &'a ast::Field, errors: &mut ErrorCollection) {
        if self.contains_key(key) {
            errors.push(ValidationError::new_duplicate_field_error(parent_name, key, &field.span));
        } else {
...
{
    fn check_duplicate(&mut self, parent_name: &str, key: &'a str, arg: &'a ast::Argument, errors: &mut ErrorCollection) {
        if self.contains_key(key) {
            errors.push(ValidationError::new_duplicate_config_key_error(parent_name, key, &arg.span));
        } else {
            self.insert(key, arg);
        }
    }
