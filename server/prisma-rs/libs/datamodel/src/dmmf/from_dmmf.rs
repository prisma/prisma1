...
pub fn default_value_from_serde(container: &Option<serde_json::Value>, field_type: &dml::FieldType) -> Option<dml::Value> {
    match (container, field_type) {
        (Some(value), dml::FieldType::Base(scalar_type)) => Some(match (value, scalar_type) {
            ...
            (serde_json::Value::Number(val), dml::ScalarType::Decimal) => dml::Value::Decimal(val.as_f64().unwrap() as f32),
            (serde_json::Value::String(val), dml::ScalarType::DateTime) => dml::Value::DateTime(String::from(val.as_str()).parse::<DateTime<Utc>>().unwrap()),
            _ => panic!("Invalid type/value combination for default value.")
        }),
        (Some(value), dml::FieldType::Enum(_)) => Some(dml::Value::ConstantLiteral(String::from(value.as_str().unwrap()))),
...
fn get_field_type(field: &Field) -> dml::FieldType {
    match &field.kind as &str {
        // TODO: Relation info in DMMF.
        "relation" => dml::FieldType::Relation( dml::RelationInfo { to: field.field_type.clone(), to_fields: vec![], name: None, on_delete: dml::OnDeleteStrategy::None } ),
...
pub fn field_from_dmmf(field: &Field) -> dml::Field {
    ...
    let id_info = match &field.is_id {
        true => Some(dml::IdInfo { strategy: dml::IdStrategy::Auto, sequence: None }),
        false => None
    };
